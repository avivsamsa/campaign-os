            ...
            <Pressable style={[s.row, { flexDirection: rowDir, borderRightColor: color(st) }]} onPress={() => router.push(`/lead/${item.id}`)}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={s.meta} numberOfLines={1}>
                  ...
                  {item.created_at ? ` · ${new Date(item.created_at).toLocaleDateString('he-IL', { day: '2-digit', month: '2-digit' })} ${new Date(item.created_at).toLocaleTimeString('he-IL', { hour: '2-digit', minute: '2-digit' })}` : ''}
